...
% ROR: Alan, need to add a green arrow back from CA1 -> ECout in first quarter!
% and ECin -> ECout in the Fourth quarter, and green arrow from ECout -> CA1 too.
% also, rename to "Fourth Quarter" instead of "Final" -- otherwise a bit inconsistent.
...
% ROR: replace big -> large -- small, medium, large is standard terminology, used above.
% Alan, replace in figures!  also font sizes should be bigger in figures so they can be read
% when shrunken down as journals tend to do
...
% ROR: Alan, it is weird to have fractional epoch values on the X axis, as Epoch is always an
% integer.  Also all these figures also need larger font size for everything to be visible when small.
% also I found that they look very fuzzy and low-res on overleaf, but much crisper on my mac.
...
In the following subsections, we consider other models of the hippocampus that have also explored some form of error-driven learning or signaling that could be related in some way to our model.  These other models are mostly outside of the predominant Hebb-Marr paradigm for understanding the hippocampus as a rapidly-learning episodic memory system, and instead suggest an alternative function for the system overall.  By contrast, our framework retains the major tenents of the Hebb-Marr paradigm, while incorporating error-driven learning to optimize the learning capacity of the system relative to the predominant use of Hebbian learning in other models.  Thus, these other models are generally incompatible with our model in their overall framing and hypotheses, but nevertheless it is useful to note the more specific aspects of similarity and differences in the underlying mechanisms.

\subsection{Scalar Novelty / Surprise Comparator Models}

First, there is a longstanding and influential set of proposals centered around the idea that the hippocampus acts as a \emph{comparator}, to detect and signal \emph{novelty} or \emph{surprise} \citep{Gray82,Vinogradova01,LismanGrace05}.  Specifically, the hippocampus in these models generates a global \emph{scalar} signal as a function of the relative mismatch between a predicted state and the actual next state.  For example \citet{Gray82} proposed that combining previous sensory information and the motor plan creates predictions about the current state, which are then compared with the actual current sensory information.  The motor plan is maintained if the two states match, but it is interrupted in the case of incorrect predictions (i.e., surprise or novelty), so that the animal can attempt to solve the problem in a different way.  Each of these different models proposes a slightly different role for the hippocampal areas in computing the mismatch signal, but they share the same overall functional role.  In the \citet{LismanGrace05} model, the hippocampal novelty or surprise signal is hypothesized to drive phasic dopamine firing via its subcortical projection pathway through the subiculum.

It is possible that some form of surprise or novelty signal could also be read out of the temporal-difference error signals present in our model.  For example, at different points in the theta phase cycle (Figure~\ref{fig.theremin}), area CA1 and ECout are representing the current information as encoded in CA3 and its projections into CA1, versus the bottom-up cortical state present in ECin.  The difference between these two activation states is hypothesized to drive error-driven learning in our model, but it could also potentially be converted into a global mismatch signal that would reflect the relative novelty of the current state compared to prior episodic memory learning in the CA3 of the hippocampus.  Likewise, it is possible that a similar global error signal could be computed from the temporal differences over CA3 in our model, reflecting the extent to which CA3 has learned to encode the more pattern-separated DG-driven pattern, which is likely to also reflect the relative novelty of the current input state.  We will investigate these possibilities in future research.

However, one important caveat is that we do not think that the hippocampus itself is capable of synthesizing novel predictions based on current sensory state and motor commands --- that requires a much more systematic, low-dimensional form of representations that generalize well across novel situations.  Indeed, we have predictive error-driven models of cortex, including the parietal and temporal lobe pathways, that do exactly this form of systematic prediction, leveraging unique thalamocortical circuits to do so \citep{OReillyRussinZolfagharEtAl21}.  By contrast, the hippocampal episodic memory representations are known to bind together the many different randomly-associated elements of a unique, specific event, creating a highly pattern-separated, unique representation thereof.  Thus, the kind of novelty or surprise signal that the hippocampus would be best-suited to compute is the \emph{episodic} novelty of the current situation relative to the closest-matching prior episodic memory triggered by the current situation.  In effect, the hippocampus could signal something like: ``I've never seen anything like this before!'', but not something like ``That's not what I predicted would have happened.''

This issue of prediction-based learning in the hippocampus is also relevant for another early computational model, which was based on using error-driven backpropagation learning in the context of a predictive autoencoder \citep{MyersGluck95}.  In this model, the hippocampal network learns by simultaneously attempting to recreate the current input patterns, and also predict future reinforcement outcomes.  The cortical network representations are then shaped by hippocampal training signals, similar in spirt to the scalar novelty / surprise signals.  Simulations with this model and its hippocampus-lesioned variant have been shown to replicate a wide range of conditioned behaviors in rats and rabbits \citep{GluckMyers94}, but we also showed that many of these same phenomena can also be accounted for within a more conventional episodic memory framework \citep{OReillyRudy01}. 

\subsection{Predictive Sequence Learning Models}

Another large class of hippocampal models also involves a predictive learning component, in the context of learning \emph{sequences} of stimuli or events over time \citep{Levy96,WallensteinHasselmo97,JensenLisman96,TsodyksSkaggsSejnowskiEtAl96,Rolls13,SchapiroTurk-BrowneBotvinickEtAl17}. In some of these models, the recurrent connections in area CA3 learn to associate prior time step representations with subsequent time step patterns, thus learning to predict the next state based on the current state.  While this notion of predicting states is similar to the scalar novelty / surprise models, these models differ in that they hypothesize that the output of the hippocampus is the predicted states themselves, rather than a single scalar summary of the difference or error in the overall prediction.  For example, many of these models suggest that the hippocampus can be used for planning future actions, by iteratively predicting future states that might occur by taking various actions.

As noted above, we do not think that the hippocampus is capable of synthesizing novel predictions in novel situations, and instead would be more well-suited for recalling prior episodic memories of relevance to the current state.  This is consistent with recent data showing that neural activity in the hippocampus that was previously interpreted as reflecting a predictive, planning-like process is actually reflecting the retrospective recall of prior events \citep{GillespieAstudilloMayaDenovellisEtAl21}.  Thus, it may be more plausible to imagine that synthetic planning processes driven by cortical processing in the prefrontal cortex and parietal lobes is also engaging hippocampal recall of relevant prior episodic information, rather than assuming that the hippocampus is capable of driving planning through sequence replay all by itself.  Nevertheless, the extent to which episodic memories encoded in the hippocampus may involve some kind of sequence-level information remains an open question that must be carefully investigated in the context of data showing that cortical areas in the medial parietal lobe encode the overall schema for temporally-extended events \citep{RanganathRitchey12}.

\subsection{Nonmonotonic Plasticity vs. Error-driven Learning}

The temporal difference error signals that drive learning in our model are closely related to the neural activation signals that drive nonmonotonic plasticity learning dynamics as explored by Norman and colleagues \citep{RitvoTurk-BrowneNorman19}.  Specifically, the nonmonotonic plasticity function drives LTD when activations are at a middling, above-zero level, while LTP occurs for more strongly activated neurons.  This is the same underlying learning function that we use in our error-driven learning model \citep{OReillyMunakataFrankEtAl12}, and thus it is difficult to strongly distinguish the predictions of these two models.  In particular, the conditions under which errors drive LTD in our model can be construed as being within the LTD range of the nonmonotonic plasticity function.  Thus, distinguishing between these models will require significantly more fine-grained model-comparison approaches, in relation to specific patterns of neural firing.

\subsection{Novel Predictions and Conclusions}

There are several novel, testable predictions from our model that can distinguish it from a more standard Hebbian-based model.  First, as shown in Figure~\ref{fig.test_stats}, the error-driven learning in area CA3 serves to drive pattern separation over time among otherwise similar representations, whereas the Hebbian version of the model showed increasing patterns similarity over learning.  Second, there is an increasingly large literature on the testing effect, where learning through testing with partial information (as compared to restudy of the complete original information) generates better subsequent memory \citep{LiuOReillyRanganath21}.  Even at a superficial level, this testing effect suggests the importance of an error-like learning signal, where the difference between an initial guess and subsequent correct answer is driving learning better than a situation where there is no opportunity to make that initial guess.  This explanation works well for explaining the robust effects where feedback is given during testing, but the testing effect also occurs, albeit more weakly, in no-feedback conditions.  In this case, we think that a temporal-difference error-like signal still arises between the initial first-pass recall step, versus the subsequent more considered recall that also has an opportunity to benefit from memory reactivation going out into the cortex and back into the hippocampus.  Furthermore, it is likely that some of the testing effect is due to learning directly in the cortex itself, where the hippocampus ends up driving cortical error-driven learning.  Thus, these dynamic interactions between hippocampus and neocortex are particularly fertile grounds for testing the error-driven nature of learning in both of these structures, which we hypothesize emerge out of the same basic learning mechanisms and temporally-based signals \citep{LiuOReillyRanganath21}.

In summary, we argue that learning in the hippocampus would benefit substantially from the use of temporal difference error-driven learning mechanisms, with both memory capacity and learning speed dramatically improved in our models.  Furthermore, these mechanisms can potentially explain a wide range of learning and memory phenomenon.  These mechanisms can emerge naturally out of neurophysiological properties of the hippocampal circuits, building on the basic framework for error-driven learning in the EC $\leftrightarrow$ CA1 pathway \citep{KetzMorkondaOReilly13}.  There are many further implications and applications of this work, and many important empirical tests needed to more fully establish its validity.  Hopefully, the results presented here provide sufficient motivation to undertake this important future research.
